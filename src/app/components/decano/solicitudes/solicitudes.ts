  ...
  ngAfterViewInit() {
      ...
      if(extRaw){
        ...
        extList.forEach(req => {
          if(!casos.find(c => c.id === req.id)){
            ...
            casos.unshift({
              id: req.id, titulo: req.titulo, estudiante: req.estudiante, tipo: req.tipo || 'Otro', prioridad: req.prioridad || 'Media', estado: req.estado || 'En proceso', creado: req.creado, actualizado: req.actualizado || req.creado, traza: req.traza || 'validacion', files: (req.files||[]).map(f => ({name:f.name, size: (f.size? (Math.round(f.size/1024)+' KB') : 'â€”')})), steps, current: req.current || 0
            });
          }
        });
      }
      ...
      casos.filter(c => !q || JSON.stringify(c).toLowerCase().includes(q))
        .forEach(c => {
          ...
          li.innerHTML = `
            ...
            <div class="seg-it-sub">${c.estudiante} Â· ${c.tipo} Â· ${c.prioridad} Â· Etapa ${c.current+1}/${c.steps.length}</div>
          `;
    ...
    function selectCaso(id){
      ...
      if(segTl){
        ...
        c.steps.forEach((s, idx) => {
          ...
          const roleClass = `role--${(s.role||'decano').replace(/[^A-Za-zÃÃ‰ÃÃ“ÃšÃ¡Ã©Ã­Ã³ÃºÃ‘Ã±]/g,'')}`;
          let meta = `Paso ${idx+1}`;
          if(s.started && s.done){
            const d = daysBetween(s.started, s.done);
            if (d !== null) meta += ` Â· ${d} dÃ­a${d===1?'':'s'}`;
          } else if(idx === c.current && s.started){
            const d = daysBetween(s.started, c.actualizado || new Date().toISOString().slice(0,10));
            if (d !== null) meta += ` Â· en curso Â· ${d} dÃ­a${d===1?'':'s'}`;
          }
          li.innerHTML = `
            <div class="tl-dot">${idx <= c.current && s.done ? '<i class="bi bi-check-lg"></i>' : 'â€¢'}</div>
            <div class="tl-body">
      ...
      if(rej){
        if(c.estado === 'Rechazado'){
          rej.hidden = false;
          rej.textContent = `Rechazado el ${fmtDate(c.rechazadoFecha)}. Motivo: ${c.rechazo || 'â€”'}`;
        } else {
          rej.hidden = true;
          rej.textContent = '';
        }
    ...
    function avanzarCaso(id){
      ...
      if(c.current < c.steps.length-1){
        ...
        c.estado = c.current >= c.steps.length-1 ? 'En resoluciÃ³n' : 'En proceso';
      }
      c.actualizado = now;
      renderSegList();
      selectCaso(id);
    ...
    function openFileModal(id, file){
      fileCtx = { id, file };
      if(fileMeta) fileMeta.textContent = `${file.name} Â· ${file.size}`;
