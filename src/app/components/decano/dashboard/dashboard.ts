...
export class Dashboard implements AfterViewInit {
  ngAfterViewInit() {
    ...
    function coGoToTramiteById(tid){
      showStatus('Buscando trÃ¡mite ' + tid + '...', 'info');
      setTimeout(() => window.location.href = '/decanato/solicitudes', 500);
    }
    ...
    if (coQa){
      coQa.addEventListener('click', (e)=>{
        ...
        if(act === 'new') window.location.href = '/decanato/solicitudes';
        else if (act === 'buscar') window.location.href = '/decanato/seguimiento';
        else if (act === 'config') window.location.href = '/decanato/notificaciones';
        else if (act === 'reportes') window.location.href = '/decanato/reportes';
      });
    }
  }
}
