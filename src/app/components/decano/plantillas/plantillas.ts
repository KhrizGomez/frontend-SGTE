...
export class Plantillas implements OnInit {
  ...
  crearPlantilla(): void {
    ...
    const idCarreraPlantilla = idPlantillaEditando
      ? (this.plantillaSeleccionada()?.idCarrera
        ...
        ?? null)
      : null;
    const idCarreraContexto = this.plantillas().find((item) => item.idCarrera > 0)?.idCarrera
      ?? this.usuariosAsignables().find((item) => (item.idCarrera ?? 0) > 0)?.idCarrera
      ?? null;
    const idCarrera = usuario?.idCarrera ?? idCarreraPlantilla ?? idCarreraContexto;
    ...
    if (!nombrePlantilla || !formulario.idCategoria || !idCarrera) {
      ...
      this.toastService.show(
        'Datos incompletos',
        idPlantillaEditando
          ? 'Completa el nombre, la categoría y selecciona un flujo. No se encontró la carrera asociada a esta plantilla.'
          : (esFlujoPersonalizado
              ? 'Completa el nombre, la categoría y define el flujo personalizado. No se encontró carrera asociada para guardar.'
              : 'Completa el nombre de la plantilla, la categoría y selecciona un flujo. No se encontró carrera asociada para guardar.'),
        'warning'
      );
      return;
    }
  ...
  cargarPlantillas(): void {
    const usuario = this.authService.obtenerUsuarioActual();
    if (!usuario?.idFacultad) {
      this.error.set('No se pudo obtener la facultad del usuario.');
      return;
    }
    ...
    const peticion$ = this.tramitesService.getPlantillasPorFacultad(usuario.idFacultad);
