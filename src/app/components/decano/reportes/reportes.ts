...
export class Reportes implements AfterViewInit {
  ngAfterViewInit() {
    ...
    const flowMap = {
      basico: [
        {name:'RecepciÃ³n', role:'decano'},
        {name:'RevisiÃ³n', role:'decano'},
        {name:'AprobaciÃ³n', role:'Decano'},
        {name:'NotificaciÃ³n', role:'decano'},
        {name:'Finalizado', role:'decano'}
      ],
      validacion: [
        {name:'RecepciÃ³n', role:'decano'},
        {name:'ValidaciÃ³n documental', role:'decano'},
        {name:'RevisiÃ³n coordinaciÃ³n', role:'decano'},
        {name:'AprobaciÃ³n decanato', role:'Decano'},
        {name:'Finalizado', role:'decano'}
      ],
      homologacion: [
        {name:'RecepciÃ³n', role:'decano'},
        {name:'EvaluaciÃ³n de homologaciÃ³n', role:'decano'},
        {name:'ResoluciÃ³n', role:'Decano'},
        {name:'NotificaciÃ³n', role:'decano'},
        {name:'Finalizado', role:'decano'}
      ],
    };
    ...
    let casos = [
      { id:'C-2101', titulo:'Cambio de carrera - Juan PÃ©rez', estudiante:'Juan PÃ©rez', tipo:'Cambio de carrera', prioridad:'Alta', estado:'En proceso', creado:'2025-09-02', actualizado:'2025-09-04', traza:'validacion', files:[ {name:'Solicitud.pdf', size:'120 KB'}, {name:'HistorialAcademico.pdf', size:'340 KB'} ], steps: flowMap.validacion.map((s,idx)=>({ ...s, started: idx===0? '2025-09-02': null, done: idx<1? '2025-09-03': null })), current: 1 },
      { id:'C-2102', titulo:'HomologaciÃ³n de materias - MarÃ­a LÃ³pez', estudiante:'MarÃ­a LÃ³pez', tipo:'HomologaciÃ³n', prioridad:'Urgente', estado:'En proceso', creado:'2025-09-01', actualizado:'2025-09-05', traza:'homologacion', files:[ {name:'Programas.pdf', size:'1.2 MB'} ], steps: flowMap.homologacion.map((s,idx)=>({ ...s, started: idx===0? '2025-09-01': null, done: idx<2? (idx===0? '2025-09-02':'2025-09-04') : null })), current: 2 },
      { id:'C-2103', titulo:'Certificado de matrÃ­cula - Carlos Ruiz', estudiante:'Carlos Ruiz', tipo:'Certificados acadÃ©micos', prioridad:'Media', estado:'En proceso', creado:'2025-09-03', actualizado:'2025-09-03', traza:'basico', files:[ {name:'Comprobante.pdf', size:'85 KB'} ], steps: flowMap.basico.map((s,idx)=>({ ...s, started: idx===0? '2025-09-03': null, done: idx<1? '2025-09-03' : null })), current: 1 }
    ];

    let rows = [
      { id: 'T-1001', titulo: 'HomologaciÃ³n de materias', tipo:'HomologaciÃ³n', prioridad:'Alta', estado:'Publicado', fecha:'2025-09-25', traza:'homologacion', custom:[] },
      { id: 'T-1002', titulo: 'Certificados acadÃ©micos', tipo:'Certificados acadÃ©micos', prioridad:'Media', estado:'En revisiÃ³n', fecha:'2025-09-20', traza:'basico', custom:[] },
      ...
      { id: 'T-1004', titulo: 'Baja de matrÃ­cula parcial', tipo:'Baja de matrÃ­cula', prioridad:'Media', estado:'Borrador', fecha:'2025-09-30', traza:'basico', custom:[] },
      { id: 'T-1005', titulo: 'Solicitud de beca institucional', tipo:'Otro', prioridad:'Urgente', estado:'En revisiÃ³n', fecha:'2025-09-18', traza:'validacion', custom:[] },
      { id: 'T-1006', titulo: 'ActualizaciÃ³n de datos personales', tipo:'Otro', prioridad:'Baja', estado:'Publicado', fecha:'2025-10-10', traza:'basico', custom:[] },
    ];
    ...
    function getHistory(){
      ...
      casos.forEach(c => {
        ...
        hist.push({
          ...
          dias: durationDays(c.creado, resolved), sla: (c.tipo==='HomologaciÃ³n'? 10 : 5), estudiante: c.estudiante
        });
      });
      rows.slice(0,6).forEach((r,i)=>{
        ...
        hist.push({ id: 'H-'+r.id, titulo: r.titulo, estudiante: 'Estudiante '+i, tipo: r.tipo, prioridad: r.prioridad, estado:'Finalizado', creado, resuelto, dias, sla:(r.tipo==='HomologaciÃ³n'?10:5) });
      });
    ...
    function renderReports(){
      ...
      const procesando = data.filter(d => d.estado==='En proceso' || d.estado==='Borrador' || d.estado==='Publicado' || d.estado==='En revisiÃ³n');
      ...
      if(repTable){
        ...
        data.slice(0, limit).forEach(d => {
          ...
          tr.innerHTML = `<td>${d.id}</td><td>${d.estudiante}</td><td>${d.tipo}</td><td>${d.creado||'â€”'}</td><td>${d.estado}</td><td>${d.sla}</td>`;
    ...
    function exportCSV(){
      ...
      const data = applyRepFilters(all);
      const rowsCsv = [
        ['ID','Estudiante','Tipo','Prioridad','Estado','Creado','Resuelto','DÃ­as','SLA']
